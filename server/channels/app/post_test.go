...
func TestSearchPostsForUser_HyphenatedUsernames(t *testing.T) {
	th := Setup(t).InitBasic()
	defer th.TearDown()

	// Create users with hyphenated usernames
	user1 := th.CreateUser()
	user1.Username = "user-123"
	user1, err := th.App.UpdateUser(th.Context, user1, true)
	require.Nil(t, err)

	user2 := th.CreateUser()
	user2.Username = "user-456" 
	user2, err = th.App.UpdateUser(th.Context, user2, true)
	require.Nil(t, err)

	// Add both users to the same team and channel
	th.LinkUserToTeam(user1, th.BasicTeam)
	th.LinkUserToTeam(user2, th.BasicTeam)
	th.AddUserToChannel(user1, th.BasicChannel)
	th.AddUserToChannel(user2, th.BasicChannel)

	// Create post mentioning user-123
	postMentioningUser123, err := th.App.CreatePost(th.Context, &model.Post{
		UserId:    th.BasicUser.Id,
		ChannelId: th.BasicChannel.Id,
		Message:   "Hello @user-123 how are you?",
	}, th.BasicChannel, model.CreatePostFlags{SetOnline: true})
	require.Nil(t, err)

	// Create post mentioning user-456
	postMentioningUser456, err := th.App.CreatePost(th.Context, &model.Post{
		UserId:    th.BasicUser.Id,
		ChannelId: th.BasicChannel.Id,
		Message:   "Hi @user-456 nice to meet you!",
	}, th.BasicChannel, model.CreatePostFlags{SetOnline: true})
	require.Nil(t, err)

	// Search for posts mentioning user-123 (this should only return the first post)
	searchTerm := "@user-123"
	results, err := th.App.SearchPostsForUser(th.Context, searchTerm, user1.Id, th.BasicTeam.Id, false, false, 0, 0, 20)
	require.Nil(t, err)

	// Verify that only the post mentioning user-123 is returned, not the one mentioning user-456
	assert.Equal(t, 1, len(results.PostList.Posts), "Should only return posts mentioning user-123")
	
	foundPost := false
	for _, post := range results.PostList.Posts {
		if post.Id == postMentioningUser123.Id {
			foundPost = true
		}
		// Ensure the post mentioning user-456 is NOT included
		assert.NotEqual(t, postMentioningUser456.Id, post.Id, "Should not return posts mentioning user-456 when searching for user-123")
	}
	assert.True(t, foundPost, "Should find the post mentioning user-123")

	// Search for posts mentioning user-456 (this should only return the second post)
	searchTerm = "@user-456"
	results, err = th.App.SearchPostsForUser(th.Context, searchTerm, user2.Id, th.BasicTeam.Id, false, false, 0, 0, 20)
	require.Nil(t, err)

	// Verify that only the post mentioning user-456 is returned, not the one mentioning user-123
	assert.Equal(t, 1, len(results.PostList.Posts), "Should only return posts mentioning user-456")
	
	foundPost = false
	for _, post := range results.PostList.Posts {
		if post.Id == postMentioningUser456.Id {
			foundPost = true
		}
		// Ensure the post mentioning user-123 is NOT included
		assert.NotEqual(t, postMentioningUser123.Id, post.Id, "Should not return posts mentioning user-123 when searching for user-456")
	}
	assert.True(t, foundPost, "Should find the post mentioning user-456")
}
